...
function [maxThree, MinofMax, minfa, AvMax, StDmax] = IRT_plot_min_max_automatic_1(maxeye, mineye, myfilepath, filename, filenames, auto_semi)
%% Load the data
...
while usr == 'r'
   ...
   %unused    % [rmin,cmin,vmin] = ind2sub(size(frame_array),find(frame_array == minfa));
 ...
 %%!       %!!fix (1)
        ...
        %place marker at max value
        hold on
        maxframe=plot(cmaxI(1),rmaxI(1),'r*', 'MarkerSize', 2)
        
        axis square;
        title({filename(2:end), [' max-' num2str(outrow), ', frame= ' num2str(maxThreeIncl(outrow,1)), ', max value= ' num2str(maxThreeIncl(outrow,2))]});
        %title({filename, strcat('\ max\-', num2str(outrow))})
        outrow=outrow+1;
        movegui('west');  
